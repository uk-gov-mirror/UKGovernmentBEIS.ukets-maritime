...
class SiteVisitWaitForAmendsInitializerTest {

    @InjectMocks
    private SiteVisitWaitForAmendsInitializer initializer;
    ...
    void initializePayload() {
        ...
        when(mapper.toApplicationReviewRequestTaskPayload(requestPayload,
                MrtmRequestTaskPayloadType.SITE_VISIT_APPLICATION_REVIEW_PAYLOAD, sectionsCompleted)).thenReturn(taskPayload);
        ...
        verify(mapper).toApplicationReviewRequestTaskPayload(requestPayload,
            MrtmRequestTaskPayloadType.SITE_VISIT_APPLICATION_REVIEW_PAYLOAD, sectionsCompleted);
    ...
    void getRequestTaskTypes() {
        assertThat(initializer.getRequestTaskTypes()).containsExactlyInAnyOrder(
            MrtmRequestTaskType.SITE_VISIT_WAIT_FOR_AMENDS);
    }
}