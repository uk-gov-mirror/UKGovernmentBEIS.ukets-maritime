...
@Injectable()
export class SiteVisitAmendsApiService extends TaskApiService<SiteVisitCommonTaskPayload> {
  private readonly pendingRequestService = inject(PendingRequestService);
  private readonly businessErrorService = inject(BusinessErrorService);

  private get saveActionTypes() {
    return {
      actionType: 'SITE_VISIT_SAVE_APPLICATION_AMEND',
      actionPayloadType: 'SITE_VISIT_SAVE_APPLICATION_AMEND_PAYLOAD',
    };
  }

  private get submitActionTypes(): SaveActionTypes {
    return {
      actionType: 'SITE_VISIT_SUBMIT_APPLICATION_AMEND',
