...
describe('mapEmissionSources', () => {
  it('maps the B5 list completion procedure and defaults emissionCompliance/emissionFactors to not exist', () => {
    ...
    expect(emissionSources.emissionFactors).toEqual({ exist: false });
    expect(emissionSources.emissionCompliance).toEqual({ exist: false });
  });

  it('marks emissionFactors as existing when a B8 procedure is present', () => {
    const { emissionSources } = mapEmissionSources({
      fuelTypes: { procedureAnnexITableB8: { referenceExistingProcedure: 'REF-B8', procedures: 'Factors' } },
    });

    expect(emissionSources.emissionFactors.exist).toBe(true);
